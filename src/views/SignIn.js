const template = `
    <section aria-label="Sign In Form" class="w-75 mx-auto color-dark-blue">
        <div class="card py-2">
            <div class="card-head text-center d-flex justify-content-center">
                <h1 class="text-black">Sign In</h1>
            </div>
            <div class="card-body">
                <form action="#" @submit.prevent="signIn" class="w-75 mx-auto">
                    <Transition name="alert" >
                          <div v-show="other_error" class="alert alert-danger" >
                            {{other_error}}
                        </div>
                    </Transition>
...
export default{
    ...
    setup(){
        ...
        async function signIn(){
            try{
                const response = await fetch('http://localhost/ws2022/module_c_solution/api/v1/auth/signin' ,{
                    method: 'POST',
                    headers:{
                ...
                else if(data.status.includes('blocked') || data.status.includes('invalid')){
                    other_error.value = data.message + " (" + data.reason + ")";
                    setTimeout(()=>{
